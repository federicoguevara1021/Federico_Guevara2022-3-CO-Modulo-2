...
class Game:
    ...
    def show_menu(self):
        if self.death_count == 0:
            ...
            font = pygame.font.Font(FONT_STYLE, 30)
            text = font.render("GAME OVER. Press any key to restar. ", True, (0, 0, 0))
            text_rect = text.get_rect()
            ...
            hscore_text = font.render(f"Highest score: {self.high_score}",True,(0,0,0))
            hscore_text_rect = hscore_text.get_rect()
            hscore_text_rect.x = (half_screen_width-100)
            hscore_text_rect.y = (half_screen_heigth+100)

            deaths_text =font.render(f"Total deaths: {self.death_count}",True,(0,0,0))
